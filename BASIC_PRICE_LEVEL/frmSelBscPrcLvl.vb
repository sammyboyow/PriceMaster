...
Public Class frmSelBscPrcLvl
    Dim vFileName As String = ""
    Public tmpmodbscpricedg As DataGridView
    ...
    Private Sub frmImprtBscPrice_KeyDown(sender As Object, e As KeyEventArgs) Handles Me.KeyDown
        If e.KeyCode = Keys.F4 Then
            If MsgBox("Do you want to remove non senior SPLIST [Y/N]?", MsgBoxStyle.YesNo + MsgBoxStyle.Information, "Price Master System") = MsgBoxResult.Yes Then
                For x As Integer = D.Rows.Count - 1 To 0 Step -1
                    If D.Rows(x).Cells("IS_SENIOR").Value = "1" Then
                        D.Rows.Remove(D.Rows(x))
                    End If
                Next
            End If
        ElseIf e.KeyCode = Keys.F3 Then
            If MsgBox("Do you want to remove member SPLIST [Y/N]?", MsgBoxStyle.YesNo + MsgBoxStyle.Information, "Price Master System") = MsgBoxResult.Yes Then
                For x As Integer = D.Rows.Count - 1 To 0 Step -1
                    If D.Rows(x).Cells("IS_MEMBER").Value.ToString = "1" Then
                        D.Rows.Remove(D.Rows(x))
                    End If
                Next
            End If
    ...
    Private Sub btnLoad_Click(sender As Object, e As EventArgs) Handles btnLoad.Click
        ...
        If cboPricelvl.Text <> "" And cboPricelvl.Text <> "ALL" Then str += str & " AND P.PRICELEVEL = '" & cboPricelvl.Text & "' "
        If cboPricelist.Text <> "" And cboPricelist.Text <> "ALL" Then str += str & " AND (P.PRICELIST LIKE '%" & cboPricelist.Text & "%' OR P.PRICELIST_DESC LIKE '%" & cboPricelist.Text & "%') "
        If chkSendinOnly.Checked Then
            str += " AND ISNULL(IS_SENDIN, 0) = 1 "
        Else

            If ChkSenior.Checked Then str += "  " Else str += " AND ISNULL(IS_SENIOR, 0) = 0 "
            If chkSendin.Checked Then str += "  " Else str += " AND ISNULL(IS_SENDIN, 0) = 0 "
        End If

        str =
            "Select P.PriceLevel,P.PriceList,P.PriceList_Desc,P.Regular_1,P.Regular_2,
                    P.Special_1,P.Special_2,P.Imaging_1,P.Imaging_2,P.UTZ_1,P.UTZ_2,
                    ISNULL(P.IS_SENIOR,0)IS_SENIOR,ISNULL(P.IS_MEMBER,0)IS_MEMBER,ISNULL(P.IS_SENDIN,0)IS_SENDIN
            FROM dbo.Price_Level_Dtl P WITH(NOLOCK) 
            LEFT JOIN dbo.PRICE_LEVEL_ORDER L ON L.PRICELEVEL = P.PRICELEVEL 
            WHERE 1 = 1 AND P.PRICELEVEL NOT IN ('MEGA4','MEGA5')  " & str & "
            ORDER BY L.ORDR "
        cmd = New SqlCommand(str, cn)
        cmd.CommandTimeout = 0
        dr = cmd.ExecuteReader
        ...
        If dr.HasRows Then
            While dr.Read
                ...
                D("IS_MEMBER", i).Value = dr("IS_MEMBER")
                D("IS_SENDIN", i).Value = dr("IS_SENDIN")
                ...
                'End If
                If dr("IS_SENIOR") = "1" Then
                    D.Rows(i).DefaultCellStyle.BackColor = Color.YellowGreen
                End If
                If dr("IS_MEMBER").ToString = "1" Then
                    D.Rows(i).DefaultCellStyle.BackColor = Color.Pink
                End If
                If dr("IS_SENDIN").ToString = "1" Then
    ...
    Private Sub btnImport_Click(sender As Object, e As EventArgs) Handles btnImport.Click
        ...
        End If
    End Sub
    Private Sub ImportTestBscPrice()
        ...
        For i As Integer = 0 To tmpmodbscpricedg.Rows.Count - 1
            If tmpmodbscpricedg("Chk", i).Value = True Then
                If tcode = "" Then
                    tcode = "'" & tmpmodbscpricedg("TestCode", i).Value & "'"
        Else
                    tcode += ",'" & tmpmodbscpricedg("TestCode", i).Value & "'"
                End If
            End If
        ...
        Dim plvl As String = ""
        Dim currplvl As String = ""
        For x As Integer = 0 To D.Rows.Count - 1
            If plvl = "" Then
                plvl = "'" & D("PriceLevel", x).Value & "'"
                currplvl = D("PriceLevel", x).Value
            Else
                If currplvl <> D("PriceLevel", x).Value Then
                    plvl += ",'" & D("PriceLevel", x).Value & "'"
                    currplvl = D("PriceLevel", x).Value
                End If
            End If
        ...
        str =
            ...
            "FROM BSC_PRICELVL_PROFILE_DTL WHERE IMH_CODE IN (" & tcode & ") AND DOCENTRY = '" & lblDocEntry.Text & "'"
        ...
        Using bulkCopy As New SqlBulkCopy(cn)
            ...
            End With
        End Using
        str = "DELETE TMPBASIC_PRICE_LVL WHERE PRICE_LEVEL NOT IN (" & plvl & ")"
        cmd = New SqlCommand(str, cn)
        cmd.ExecuteNonQuery()
    ...
    'IMPORTEXCEL FUNCTION BACKUP ORIGINAL
    '    Dim dt As New DataTable
    ...
    Private Sub Generate()
        ...
        Try
            ...
            'If chkNewProc.Checked = True Then
            '    str =
            '    "EXEC CREATE_IMHN '" & Info.POSID & "', '" & dtpefd.Value & "', '" & imhbid & "'"
            'Else
            '    str =
            '    "EXEC CREATE_IMH '" & Info.POSID & "', '" & dtpefd.Value & "', '" & imhbid & "'"
            'End If
            str = "EXEC CREATE_IMHN '" & Info.POSID & "', '" & dtpefd.Value & "', '" & imhbid & "'"
            cmd = New SqlCommand(str, cn)
            cmd.CommandTimeout = 0
            cmd.ExecuteNonQuery()

            str = "EXEC CREATE_SPLIST '" & Info.POSID & "', '" & dtpefd.Value & "', '" & spbid & "'"
            cmd = New SqlCommand(str, cn)
            cmd.CommandTimeout = 0
            cmd.ExecuteNonQuery()
    ...
    Private Sub chkSendinOnly_CheckedChanged(sender As Object, e As EventArgs) Handles chkSendinOnly.CheckedChanged
        If chkSendinOnly.Checked Then
            ChkSenior.Checked = False
            chkSendin.Checked = False
        End If
    End Sub
End Class